...
var BuildHTML = function () {
  ...
  BuildHTML.prototype._build = function _build(text, who) {
    return '<div class="' + this.messageWrapper + ' ' + this[who + 'Class'] + '">\n              <div class=" profile profilePic '+ this[who + 'Class'] +' animated zoomIn"><i src="https://s3.amazonaws.com/whisperinvest-images/default.png" alt="" style="width:10px;height:10px"></div>\n              <div class="' + this.textWrapper + '">...</div>\n            </div>';
  };
...
$(document).ready(function () {
  ...
  function sendMessage() {
    var textmsg = $input.val(),
    params = "textmsg=" + textmsg + "&otherparam=1",
    xmlhttp = new XMLHttpRequest(), 
    response = '';
    messenger.send(textmsg);
   

   xmlhttp.open("POST","http://localhost:7171/message", true);
   xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
   xmlhttp.onreadystatechange=function(){
      if (xmlhttp.readyState==4 && xmlhttp.status==200){
         response = xmlhttp.responseText+'';
         messenger.recieve(response);
      }
   }
